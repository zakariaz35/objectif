...
<template>
  <div class="shell">
    <header class="topbar">
      ...
      <nav class="account">
        <RouterLink to="/explorer" class="nav-link" :title="$t('nav.exploreTitle')">📚 {{ $t('nav.explore') }}</RouterLink>
        <RouterLink to="/plan" class="nav-link" :title="$t('nav.planTitle')">🎯 {{ $t('nav.plan') }}</RouterLink>
        <RouterLink to="/reviser" class="nav-link" :title="$t('nav.reviewTitle')">🔁 {{ $t('nav.review') }}</RouterLink>
        <RouterLink to="/tableau-de-bord" class="nav-link" :title="$t('nav.dashboardTitle')">📊 {{ $t('nav.dashboard') }}</RouterLink>
        <label class="lang" :title="$t('lang.select')">
          🌐
          <select :value="locale" @change="setLocale($event.target.value)">
            <option v-for="l in LOCALES" :key="l.code" :value="l.code">{{ l.label }}</option>
          </select>
        </label>
        <label class="theme" :title="$t('app.theme')">
          🎨
        ...
        <RouterLink to="/logos" class="debug-link" :title="$t('nav.logosTitle')">✦ {{ $t('nav.logos') }}</RouterLink>
        <!-- ⚠️ DEBUG / personal use only — link to the accounts list. Remove if not personal. -->
        <RouterLink to="/comptes" class="debug-link" :title="$t('nav.accountsTitle')">⚙ {{ $t('nav.accounts') }}</RouterLink>
        <template v-if="auth.isAuthenticated.value">
          <span class="who">👤 {{ auth.state.user?.name }}</span>
          <button class="btn-link" @click="logout">{{ $t('nav.logout') }}</button>
        </template>
        <RouterLink v-else to="/auth" class="signin">{{ $t('nav.signIn') }}</RouterLink>
      </nav>
    </header>
    <RouterView />
