...
async function load() {
  ...
  try {
    ...
    if (id !== reqId) return
    error.value = t('formation.notFound')
  } finally {
...
const typeIcon = { lesson: '🥄', exercise: '🛠️', quiz: '🎯', flashcards: '🃏', matching: '🔗', cloze: '✏️' }
...
<template>
  <div class="layout">
    <nav class="toc" ref="tocEl">
      ...
      <RouterLink :to="`/f/${formation}/resultats`" class="results-link">{{ $t('formation.myResults') }}</RouterLink>
    ...
    <main class="content">
      ...
      <template v-else>
        <RouterView v-if="route.name === 'lesson' || route.name === 'results'" />
        <div v-else class="welcome">
          ...
          <p class="rpct">{{ $t('formation.overallProgress', { done: doneCount, total: totalLessons, pct: percent }) }}</p>
          ...
          <ol class="steps">
            <li v-for="(s, i) in roadmap" :key="s.slug" class="step" :class="s.status">
              ...
              <div class="sbody">
                ...
                <div class="smeta">{{ $t('formation.lessonsCount', { done: s.done, total: s.total }) }}</div>
              </div>
              <RouterLink v-if="s.to" :to="s.to" class="btn btn-primary sbtn">
                {{ s.status === 'todo' ? $t('formation.start') : s.status === 'done' ? $t('formation.reviewAgain') : $t('formation.resume') }}
              </RouterLink>
            </li>
          </ol>
        </div>
...
.results-link:hover,
.results-link.router-link-active {
  background: var(--panel2);
  color: var(--txt);
