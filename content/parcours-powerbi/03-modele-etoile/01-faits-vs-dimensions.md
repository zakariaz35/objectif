...
```mermaid
flowchart LR
    subgraph Fait["Table de faits — longue & étroite"]
        F["Sales<br/>des milliers de lignes<br/>peu de colonnes (clés + mesures)"]
    end
    subgraph Dims["Tables de dimension — courtes & larges"]
        D1["Products<br/>quelques centaines de lignes<br/>beaucoup de colonnes descriptives"]
        D2["Customers<br/>quelques milliers de lignes<br/>beaucoup de colonnes descriptives"]
    end
    D1 -->|"1 → *"| F
    D2 -->|"1 → *"| F
```

Ce contraste **long/étroit vs court/large** est le premier réflexe avant de dessiner l'étoile complète (relations, module suivant) : beaucoup de lignes et peu de colonnes d'un côté, peu de lignes et beaucoup de colonnes de l'autre.
