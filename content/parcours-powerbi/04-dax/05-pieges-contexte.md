...
**Chiffres à l'appui** — 3 lignes de `Sales` :

| `amount` | `quantity` | `Ratio Per Row` (colonne calculée) |
|---|---|---|
| 100 | 10 | 10 |
| 300 | 20 | 15 |
| 90 | 9 | 10 |
| **Total (SUM implicite)** | | **35** ← `10 + 15 + 10`, une somme de ratios, sans aucun sens métier |

Le bon ratio global — `DIVIDE(SUM(amount), SUM(quantity))` = `490 / 39` ≈ **12,6** — ne correspond ni au total affiché (35), ni même à la moyenne des trois ratios (11,7). Seule la mesure atomique donne le bon chiffre.
...
Une valeur **négative** signale des dates manquantes dans la table ; une valeur **positive** signale des doublons ou des lignes en trop.
