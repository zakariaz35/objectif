...
cards:
  ...
  - q: |
    ...
    a: |
      ...
      le contexte. `DATEADD(date, -1, MONTH)` décale d'un **mois calendaire** (même jour
      du mois, ajusté pour les mois plus courts) — plus flexible pour d'autres unités
      (jour, trimestre, année) mais pas forcément aligné sur un mois plein si le contexte
      ne couvre pas déjà tout le mois. Pour comparer des mois complets, `PREVIOUSMONTH`
      reste le plus direct.
  - q: |
