...
```mermaid
flowchart LR
    Src["Source<br/>(fichier brut)"] --> S1["Étape : Type des colonnes"]
    S1 --> S2["Étape : Nettoyer le texte"]
    S2 --> S3["Étape : Colonnes personnalisées"]
    S3 --> Out["Table propre<br/>(rejouée à chaque actualisation)"]
```

Chaque étape porte un nom dans le volet *Étapes appliquées* et transforme la sortie de la précédente. Au clic sur *Actualiser*, Power Query **rejoue exactement cette séquence** sur les données fraîches — c'est ce qui rend la transformation reproductible.
...
## Une ligne brute, étape par étape

Prenons une ligne réelle d'un export mal formaté, et suivons-la à travers les étapes du volet *Étapes appliquées* :
...
| Étape | Ce qu'elle fait | `name` | `amount` | `order_date` |
|---|---|---|---|---|
| Source | données telles quelles | `" DUPONT jean "` | `"1 234,50 €"` | `"01/02/2024"` |
| Trim + Proper (`name`) | retire les espaces, met en forme | `"Dupont Jean"` | `"1 234,50 €"` | `"01/02/2024"` |
| Nettoyer texte (`amount`) | retire `" €"`, remplace `,` par `.` | `"Dupont Jean"` | `"1234.50"` | `"01/02/2024"` |
| Changer de type (`amount` → Nombre décimal) | convertit en nombre | `"Dupont Jean"` | `1234.50` | `"01/02/2024"` |
| Changer de type (`order_date` → Date, régional FR) | interprète `JJ/MM/AAAA` | `"Dupont Jean"` | `1234.50` | `2024-02-01` |

Chaque étape est **rejouée automatiquement** au prochain rafraîchissement, sur toutes les nouvelles lignes qui ont la même forme brute — c'est tout l'intérêt de nettoyer une fois dans Power Query plutôt que dans chaque visuel.
