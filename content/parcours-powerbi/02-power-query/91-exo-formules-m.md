...
5. **Classer la commande** en `"Large"` (montant > 500), `"Medium"` (100-500), `"Small"` (< 100) — libellés en anglais, comme dans la leçon *Nettoyer & transformer*.
...
if [unit_price] * [quantity] > 500 then "Large"
else if [unit_price] * [quantity] >= 100 then "Medium"
else "Small"
```

> Astuce : si tu as déjà créé la colonne `line_amount` à l'étape 4, utilise-la directement (`[line_amount]`) plutôt que de recalculer — Power Query exécute les étapes dans l'ordre du volet *Étapes appliquées*. Les libellés (`"Large"`/`"Medium"`/`"Small"`) restent en anglais, comme dans la colonne `Classify order size` de la leçon *Nettoyer & transformer*.
