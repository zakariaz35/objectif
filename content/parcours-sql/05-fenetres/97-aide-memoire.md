...
FROM orders
GROUP BY category
HAVING SUM(amount) > 1000   -- HAVING filters groups, not rows
ORDER BY total DESC;
```
...
-- LEFT: all rows from the left, NULL when there's no match
...
WHERE id IN (SELECT DISTINCT customer_id FROM orders WHERE region = 'North');

-- EXISTS: existence test (stops at the first match, often faster)
