  ...
  tests:
    ...
    - name: "nombre de lignes inchangé (pas de réduction)"
      code: |
        ...
        const got = categoryShare(orders)
        console.log('result size:', got.length, '(must stay 3)')
        assertEqual(got.length, 3, 'the window does not reduce the number of rows')
    - name: "catégorie à une seule ligne → categoryShare = 1"
      code: |
        ...
        const got = categoryShare(orders)
        console.log('result single category:', got)
        assert(Math.abs(got[0].categoryShare - 1.0) < 1e-9, 'single row -> share = 1.0 (100%)')
    - name: "table vide → []"
      code: |
        assertEqual(categoryShare([]), [], 'no orders -> no rows')
---
