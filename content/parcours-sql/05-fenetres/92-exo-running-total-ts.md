  ...
  tests:
    - name: "cumul croissant par date"
      code: |
        ...
        console.log('input  (unsorted)   :', orders)
        console.log('result (sorted + cumulative):', got)
        // sorted: 01/01(100), 01/02(200), 01/03(50)
        // cumul :        100,        300,       350
        assertEqual(got[0].orderDate,    '2024-01-01', 'first date comes first')
        assertEqual(got[0].runningTotal, 100, 'cumulative total after 01/01 = 100')
        assertEqual(got[1].runningTotal, 300, 'cumulative total after 01/02 = 100 + 200')
        assertEqual(got[2].runningTotal, 350, 'cumulative total after 01/03 = 300 + 50')
    - name: "tri préservé indépendamment de l'ordre d'entrée"
      code: |
        ...
        assertEqual(got[0].orderId, 1, 'orderId 1 (01/15) comes first')
        assertEqual(got[0].runningTotal, 300, 'cumulative total after 01/15 = 300')
        assertEqual(got[1].runningTotal, 800, 'cumulative total after 03/01 = 300 + 500')
    - name: "une seule commande → runningTotal = amount"
      code: |
        ...
        const got = runningTotal(orders)
        console.log('result single row:', got)
        assertEqual(got[0].runningTotal, 42, 'for a single row, cumulative total = amount')
    - name: "table vide → []"
      code: |
        assertEqual(runningTotal([]), [], 'no orders -> empty array')
---
