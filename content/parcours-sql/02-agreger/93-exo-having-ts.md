  ...
  tests:
    - name: "HAVING filtre les groupes pas les lignes"
      code: |
        ...
        // Office: 3 orders, 350 -> passes (3>=2 AND 350>=300)
        // Hardware: 1 order, 900 -> fails (1 < 2)
        // Software: 1 order, 10  -> fails
        const got = groupsWithHaving(orders, 2, 300)
        console.log('input  :', orders)
        console.log('HAVING result (minOrders=2, minRevenue=300):', got)
        assertEqual(got.length, 1, 'only Office passes both conditions')
        assertEqual(got[0].category, 'Office', 'Office is kept')
    ...
    - name: "filtre sur CA uniquement"
      code: |
        ...
        // minOrders=1, minRevenue=500 -> only Hardware (900) passes
        const got = groupsWithHaving(orders, 1, 500)
        console.log('HAVING result (minOrders=1, minRevenue=500):', got)
        assertEqual(got.length, 1, 'only Hardware passes (revenue >= 500)')
        assertEqual(got[0].category, 'Hardware', 'Hardware is kept')
    - name: "tri par CA décroissant"
      code: |
        ...
        console.log('result sorted by revenue desc:', got.map((r) => r.category))
        assertEqual(got[0].category, 'Hardware', 'highest revenue first')
        assertEqual(got[2].category, 'Software', 'lowest revenue last')
    - name: "table vide → []"
      code: |
        assertEqual(groupsWithHaving([], 1, 0), [], 'no orders -> no groups')
---
