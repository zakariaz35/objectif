...
requête qui renvoie les **10 plus grosses commandes** de la région `North`, passées
...
- `amount IS NOT NULL` n'est pas redondant ici : sous PostgreSQL, un `NULL` est traité
  comme la **plus grande** valeur possible, donc en tri `DESC` les `NULL` remontent
  **en tête** (pas en bas). D'où l'intérêt du filtre explicite — ou, à défaut,
