  ...
  tests:
    - name: "client sans commande → orderId null"
      code: |
        ...
        assertEqual(got.length, 4, '4 rows: Alice×2, Bob×1, Carol×1')
        const bob = got.find((r) => r.name === 'Bob')
        assertEqual(bob?.orderId, null, 'Bob: orderId must be null')
    - name: "trouver les clients sans commande (WHERE orderId IS NULL)"
      code: |
        ...
        console.log('customers without an order:', withoutOrder)
        assertEqual(withoutOrder.length, 1, 'only one customer without an order')
        assertEqual(withoutOrder[0].name, 'Bob', 'Bob has no order')
    - name: "aucune commande → tous les clients avec orderId null"
      code: |
        ...
        console.log('result with no orders:', got)
        assertEqual(got.length, 2, 'both customers present even without orders')
        assert(got.every((r) => r.orderId === null), 'all orderId values are null')
---
