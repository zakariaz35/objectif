...
# Typed path param: /items/42  -> item_id == 42 (int)
...
```python
# Reproduction of the type conversion FastAPI runs on a typed "query/path param".
# FastAPI validates each parameter via Pydantic. We simulate "item_id: int".
from pydantic import TypeAdapter, ValidationError

# item_id: int  ->  a TypeAdapter over int
...
for valeur in ["42", "007", "abc", "3.5"]:
    try:
        ...
        print(f"{valeur!r:>8}  ->  422 Unprocessable Entity (not an integer)")

# Query bool: FastAPI accepts true/false/1/0/yes/no...
print("---")
