...
Chaque déclaration crée un **injecteur** à un niveau différent de l'arbre : l'injecteur racine ne crée qu'**une seule** instance pour toute l'app, tandis qu'un injecteur de composant en crée une **nouvelle à chaque instance** de ce composant.

```mermaid
flowchart TD
  Root["Injecteur racine<br/>(providedIn: 'root')"] --> Shared["1 instance partagée<br/>par toute l'application"]

  subgraph Card1["ProductCardComponent (instance 1)"]
    Inj1["Injecteur de composant<br/>(providers: [X])"] --> S1["Instance dédiée n°1"]
  end

  subgraph Card2["ProductCardComponent (instance 2)"]
    Inj2["Injecteur de composant<br/>(providers: [X])"] --> S2["Instance dédiée n°2"]
  end
```

La portée composant est utile quand chaque instance d'un composant a besoin de **son propre** état isolé (ci-dessus, deux cartes produit ne partagent rien). Dans le doute, reste sur `'root'`.
