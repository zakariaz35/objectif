...
```mermaid
flowchart LR
  User(("Utilisateur")) -->|"charge l'app"| Initial["Bundle initial<br/>(HomeComponent)"]
  User -->|"visite /admin"| Chunk1["Chunk téléchargé à la volée<br/>(AdminComponent)"]
  User -->|"visite /reports"| Chunk2["Chunk téléchargé à la volée<br/>(reportsRoutes)"]
```

Seul `Initial` fait partie du chargement de démarrage ; les deux chunks n'arrivent que si l'utilisateur visite ces routes.
