...
exercise:
  language: js
  starter: |
    // You are given an ARRAY of sale amounts (in euros), of any size.
    // Write a function that returns their TOTAL, by iterating over the array with a loop.
    function totalVentes(ventes) {
      let total = 0
      // TODO: iterate over each amount in `ventes` (a for...of loop is ideal)
      //       and add it to `total`.
      return total
    }

    // Write a function that returns the AVERAGE of the sales.
    // Hint: reuse totalVentes, then divide by the number of elements (ventes.length).
    function moyenneVentes(ventes) {
      // TODO: compute the average. Watch out for the empty array case!
      return null
    }
...
function totalVentes(ventes) {
  let total = 0
  for (const montant of ventes) {   // for each amount in the array
    total = total + montant         // accumulate (accumulator pattern)
  }
  return total
}
...
function moyenneVentes(ventes) {
  if (ventes.length === 0) {
    return 0                        // safety guard: no division by 0
  }
