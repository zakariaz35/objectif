...
> 💡 **Analogie — l'accumulateur comme une caisse enregistreuse.** Avant d'ouvrir le magasin, la caisse est à **zéro** (déclaration *avant* la boucle). Chaque vente s'y additionne. À la fermeture, elle affiche le total de la journée. Si tu remettais la caisse à zéro *avant chaque vente* (déclaration *à l'intérieur* de la boucle), tu aurais toujours le montant de la dernière vente — jamais le total.

> ⚠️ **Erreur fréquente — l'erreur de décalage (*off-by-one*).** La confusion entre `<` et `<=` est un classique absolu. Avec un tableau de 5 éléments (indices 0 à 4) :
>
> - `i < 5` → tours pour i = 0, 1, 2, 3, 4 → ✅ 5 tours, tous les éléments parcourus
> - `i <= 5` → tours pour i = 0, 1, 2, 3, 4, **5** → ❌ `ventes[5]` n'existe pas : c'est `undefined`
>
> Retiens la règle : **`i < longueur`**, jamais `i <= longueur`.
...
// We withdraw 30 € per day as long as there is money left
while (solde >= 30) {
  solde = solde - 30
  jours = jours + 1
}
...
```js
// ⚠️ INFINITE LOOP — do NOT run this
let n = 0
while (n < 5) {
  console.log("Stuck...")
  // we FORGOT to advance n
}
```
...
`6` — la boucle fait 4 tours (i = 0, 1, 2, 3). L'accumulateur cumule : `0 + 0 = 0`, puis `0 + 1 = 1`, puis `1 + 2 = 3`, puis `3 + 3 = 6`.
...
Trois lignes : `apple`, puis `banana`, puis `cherry`. Le `for...of` parcourt chaque élément dans l'ordre d'insertion, sans index.
...
`32` — `n` double à chaque tour : 1 → 2 → 4 → 8 → 16 → 32. À ce point `32 < 20` est faux, la boucle s'arrête et on affiche `32`.
