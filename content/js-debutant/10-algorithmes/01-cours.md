...
let total = 0
let max = ventes[0]        // start from the first element
for (const m of ventes) {
  total = total + m        // sum accumulation
  if (m > max) {
    max = m                // maximum accumulation
  }
}
...
function trouverVille(villes, cible) {
  for (let i = 0; i < villes.length; i++) {
    if (villes[i] === cible) {
      return i          // found: return its position and EXIT
    }
  }
...
function chercheDichotomique(triee, cible) {
  ...
  while (gauche <= droite) {
    ...
    if (triee[milieu] < cible) {
      gauche = milieu + 1     // too small: discard the left half
    } else {
      droite = milieu - 1     // too large: discard the right half
    }
  }
  return -1                   // absent
}
...
function triSelection(tableau) {
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    for (let j = i + 1; j < t.length; j++) {   // boucle DANS la boucle
      if (t[j] < t[idxMin]) {
        idxMin = j                       // track the smallest of the rest
      }
    }
    const tmp = t[i]                     // swap t[i] and t[idxMin]
    t[i] = t[idxMin]
    t[idxMin] = tmp
  }
  return t
