...
console.log(villes[0])   // "Paris"  ← first slot
console.log(villes[2])   // "Nice"
console.log(villes[3])   // "Brest"  ← last slot
console.log(villes[4])   // undefined ← this slot does not exist
```
...
console.log(ventes[ventes.length - 1])  // 30  ← the last element
```
...
```js
const panier = ["bread", "milk"]

panier.push("eggs")     // adds to the END
console.log(panier)     // ["bread", "milk", "eggs"]

const dernier = panier.pop()   // removes the LAST element and returns it
console.log(dernier)    // "eggs"
console.log(panier)     // ["bread", "milk"]
```
...
// Style 1 — classic for: we have access to index i
for (let i = 0; i < ventes.length; i++) {
  console.log("Vente n°" + i + " :", ventes[i])
}

// Style 2 — for...of: we get each element directly
...
const totalGrossesAvecTva = ventes
  .filter((m) => m > 100)      // keep sales > 100 €         → [120, 200]
  .map((m) => m * 1.2)         // add 20% VAT                → [144, 240]
  .reduce((s, m) => s + m, 0)  // sum them                   → 384
