...
exercise:
  language: js
  starter: |
    // You are given an array of sale amounts (in euros).
    // You will build a small data processing pipeline.

    // 1) Returns a NEW array keeping only sales STRICTLY above the threshold.
    function ventesAuDessus(ventes, seuil) {
      // TODO: use filter
      return null
    }

    // 2) Returns a NEW array where each amount has VAT applied (× 1.2, i.e. +20%).
    function ajouterTva(ventes) {
      // TODO: use map
      return null
    }

    // 3) Returns the TOTAL, VAT included, of sales above the threshold.
    //    Hint: reuse ventesAuDessus then ajouterTva, then sum (reduce).
    function totalGrossesTtc(ventes, seuil) {
      // TODO: chain the two functions above, then reduce
      return null
    }
