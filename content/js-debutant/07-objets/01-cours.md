...
// Dot notation (most common, when the key is known at write time)
console.log(perso.nom)      // "Ada"
console.log(perso.age)      // 42

// Bracket notation (when the key is in a variable, or contains a space)
console.log(perso["nom"])   // "Ada"

const champ = "age"
console.log(perso[champ])   // 42  ← the key comes from a variable
```
...
perso.age = 43              // modify an existing property
perso.ville = "London"      // add a NEW property
delete perso.age            // delete a property

console.log(perso)          // { nom: "Ada", ville: "London" }
```
...
console.log(commande.client.nom)     // "Ada"   ← chained dot access
...
flowchart TD
  ...
  T --> L1["index 1 : { produit: 'Coffee', montant: 8, categorie: 'resto' }"]
  T --> L2["index 2 : { produit: 'Écran', montant: 200, categorie: 'info' }"]
  T --> L3["index 3 : { produit: 'Tea', montant: 6, categorie: 'resto' }"]
```
...
// Total sales in the "info" category
let totalInfo = 0
for (const vente of ventes) {
  if (vente.categorie === "info") {   // read a COLUMN from the row
    totalInfo += vente.montant        // accumulate another COLUMN
  }
}
console.log("Total info :", totalInfo)   // 320

// Same thing in pipeline style (Tableaux module)
const total2 = ventes
