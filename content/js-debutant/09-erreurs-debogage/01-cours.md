...
function moyenne(a, b, c) {
  return a + b + c / 3      // forgotten precedence: only c is divided by 3!
}
...
```js
function double(x) {
  return x.montant * 2      // ← x has no montant property if x is a number
}
...
function montantEnNombre(texte) {
  try {
    ...
    if (Number.isNaN(n)) {
      throw new Error("Invalid amount: " + texte)   // we RAISE an error
    }
    return n
  } catch (erreur) {
    console.log("Issue:", erreur.message)              // we CATCH it
    return 0                                            // fallback value
  } finally {
    console.log("Conversion attempt complete.")    // ALWAYS executed
  }
}

console.log(montantEnNombre("120"))    // 120  (try block succeeded)
console.log(montantEnNombre("abc"))    // 0    (error caught, fallback)
```
...
function moyenne(notes) {
  ...
  for (const n of notes) {
    total = total + n
    console.log("après", n, "→ total =", total)   // probe: tracking the accumulator
  }
  console.log("final total:", total, "over", notes.length, "notes")
