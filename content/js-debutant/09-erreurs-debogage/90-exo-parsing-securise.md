...
exercise:
  language: js
  starter: |
    // "sales" thread: amounts sometimes arrive as TEXT
    // (from a file, a form...). Some are invalid ("abc", "").
    // You will write ROBUST code that rejects bad values cleanly.

    // 1) Converts a text to a number. If the text is NOT a valid number,
    //    RAISE an error with throw new Error("Invalid amount: " + texte).
    //    Hint: Number(texte) gives NaN if conversion fails; test with Number.isNaN().
    function parserMontant(texte) {
      // TODO: convert; if invalid, throw; otherwise return the number
      return null
    }

    // 2) SAFE version: try to parse, and if it fails, return 0 instead of crashing.
    //    Hint: wrap the call to parserMontant in a try / catch.
    function parserMontantSecurise(texte) {
      // TODO: try { ... } catch (e) { return 0 }
      return null
    }

    // 3) Takes an ARRAY of texts and returns the TOTAL of valid amounts,
    //    invalid ones counting as 0 (thanks to parserMontantSecurise).
    function totalMontants(textes) {
      // TODO: accumulate parserMontantSecurise(each text)
      return null
    }
...
function parserMontant(texte) {
  ...
  if (Number.isNaN(n)) {
    throw new Error("Invalid amount: " + texte)   // refuse the bad input
  }
  return n
}
...
function parserMontantSecurise(texte) {
  try {
    return parserMontant(texte)          // we try...
  } catch (e) {
    return 0                             // ...and if it fails, fallback value
  }
}
...
function totalMontants(textes) {
  ...
  for (const texte of textes) {
    total = total + parserMontantSecurise(texte)   // invalid ones count as 0
  }
  return total
}
```
