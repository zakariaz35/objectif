...
*(Ici, Q3 = 4 j par la méthode du rang inférieur. Avec interpolation linéaire — méthode
par défaut de `numpy.percentile` ou d'Excel `PERCENTILE.INC` — on obtiendrait plutôt
≈ 4,25 j. Les deux méthodes existent : précise laquelle tu utilises si le chiffre exact
compte.)*
