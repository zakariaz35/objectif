...
flowchart LR
    ...
    subgraph Core["Cœur métier — zéro dépendance externe"]
        ...
        SVC["CreateInvoice (service applicatif)"]
        POUT(["Port de sortie : interface InvoiceRepository"])
        PIN --> SVC --> POUT
    end
    ELO[("EloquentInvoiceRepository")]
...
> **L'inversion de dépendance, le vrai secret —** Normalement « métier → utilise MySQL ». Ici on **inverse** : le métier définit l'interface `InvoiceRepository`, et c'est l'adapter Eloquent qui **dépend de cette interface**. La flèche de dépendance pointe vers le centre. Résultat : le centre ignore totalement Eloquent.
