---
title: Quiz éclair — JWT
type: quiz
questions:
  - prompt: |
      Un attaquant change `"role":"user"` en `"role":"admin"` dans le payload d'un JWT.
      Pourquoi le serveur rejette-t-il le token ?
    options:
      - "Parce que le payload est chiffré et donc illisible"
      - "Parce que la signature ne correspond plus au payload modifié (la recréer exigerait le secret du serveur)"
      - "Parce que la date d'expiration devient automatiquement invalide"
    answer: 1
    tags: [jwt, signature]
    level: debutant
    explanation: >
      La signature couvre le header + le payload. En modifiant le payload, la signature
      d'origine ne colle plus ; pour en forger une valide, il faudrait le SECRET du
      serveur. Le serveur recalcule, voit la différence et rejette le token.
  - prompt: |
      Une seule application émet ET vérifie ses tokens (pas d'autre service impliqué).
      Quel algorithme de signature choisir ?
    options:
      - "RS256, toujours par principe : c'est plus sécurisé"
      - "HS256 : un secret partagé suffit, inutile de gérer une paire de clés"
      - "Aucune importance, les deux sont strictement équivalents"
    answer: 1
    tags: [jwt, signature]
    level: intermediaire
    explanation: >
      HS256 (secret partagé) convient parfaitement quand la même appli émet et vérifie :
      un seul secret à protéger, simple à opérer. RS256 (paire clé privée/publique)
      n'apporte de valeur que lorsque **d'autres services** doivent pouvoir vérifier le
      token **sans** pouvoir en émettre de nouveaux (microservices, SSO) — sinon c'est de
      la complexité gratuite.
---

Deux questions pour vérifier que tu as compris le rôle de la signature et le bon choix
d'algorithme.
