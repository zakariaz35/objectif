...
```
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI0MiIsIm5hbWUiOiJIYXNzYW5lIiwicm9sZSI6ImFkbWluIiwiZXhwIjoxNzE3NTAwMDAwfQ.Bl-GRE8mCA1FRnQOxAuPBsQo7sVmmyZQnqHk-4sqg-I
```

> **⬢ Essaie-le —** Ce token est **réellement signé** (pas juste un exemple visuel) avec le secret `your-256-bit-secret` — c'est le secret par défaut proposé sur `jwt.io`. Colle le token ci-dessus dans le débogueur de `jwt.io` et renseigne ce secret dans le champ « your-256-bit-secret » : tu dois voir **« Signature Verified »**.
