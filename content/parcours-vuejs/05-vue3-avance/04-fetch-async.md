...
## À toi de jouer : une version exécutable (mockée)

Le vrai `fetch('/api/users')` ci-dessus n'existe pas dans le playground — mais on peut
**simuler le réseau** avec une fausse API (un délai via `setTimeout`, et une erreur une
fois sur trois via un compteur) pour que tu **testes la même mécanique en direct**.

```vue
<script setup>
import { ref, onMounted } from 'vue'

// Fake API: simulates network latency, and fails once every 3 calls
let callCount = 0
function fetchUsersMock() {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (callCount % 3 === 0) {
        reject(new Error('HTTP 500 (simulated failure)'))
      } else {
        resolve([
          { id: 1, name: 'Ada' },
          { id: 2, name: 'Grace' },
...
Clique **« Tester »**, puis **« Recharger »** plusieurs fois : au bout de trois clics, tu
verras l'état **erreur** s'afficher — exactement le même `try / catch / finally` que
l'exemple réel, juste branché sur un faux réseau.
