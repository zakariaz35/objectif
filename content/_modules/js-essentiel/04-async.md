...
```js
fetch('/api/users')
  .then((res) => res.json())          // when the response arrives, parse it
  .then((users) => console.log(users)) // then use it
  .catch((err) => console.error(err))  // on failure (network, etc.)
```
...
async function loadUsers() {
  try {
    const res = await fetch('/api/users')   // wait for the response
    const users = await res.json()          // wait for the parsing
    return users
  } catch (err) {
    console.error('loading failed', err)    // this is where "rejected" lands
