...
const msg = `Hello ${name}, it's ${hour}h`
console.log(msg)   // Hello Ada, it's 14h (depends on the current time)
```
...
const { name, role } = user         // two fields extracted at once
console.log(name, role)             // Ada admin

const [first, second] = [10, 20]    // same for an array (by position)
...
```js
const a = [1, 2], b = [3, 4]
const all = [...a, ...b]                 // [1, 2, 3, 4] : merge
const copy = { ...user, role: 'user' }   // copy of user + override one field
...
```js
const user = { name: 'Ada' }                 // no address here
const city = user.address?.city              // undefined instead of an error
const page = null ?? 1                        // 1 if null/undefined…
const kept = 0 ?? 1                           // …but 0 is kept → 0
