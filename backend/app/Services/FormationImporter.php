...
class FormationImporter
{
    ...
    private function importLesson(Module $module, string $file, int $defaultPos): void
    {
        ...
        $lesson = $module->lessons()->create([
            ...
            'pairs' => $this->normalizePairs($meta['pairs'] ?? null),
            'cloze' => $this->normalizeCloze($meta['cloze'] ?? null),
        ]);
    ...
    /**
     * Optional matching pairs (the "pairs" key: relier EN↔FR).
     *
     * @return list<array{en_html:string, fr_html:string}>|null
     */
    private function normalizePairs(mixed $pairs): ?array
    {
        ...
        foreach ($pairs as $p) {
            ...
            $en = $p['en'] ?? $p['left'] ?? null;
            $fr = $p['fr'] ?? $p['right'] ?? null;
            if (! $en || ! $fr) {
                continue;
            }
            $out[] = [
                'en_html' => (string) $this->inlineHtml((string) $en),
                'fr_html' => (string) $this->inlineHtml((string) $fr),
            ];
        }
    ...
    /**
     * Optional cloze exercise (the "cloze" key): a string or list of strings, each
     * containing one or more `{{answer}}` gaps (alternatives with `{{a|b}}`).
     * Stored split into interleaved text `parts` and their `answers`.
     *
     * @return array<int,array{parts:list<string>, answers:list<string>}>|null
     */
    private function normalizeCloze(mixed $cloze): ?array
    {
        if (is_string($cloze)) {
            $cloze = [$cloze];
        }
        if (! is_array($cloze) || empty($cloze)) {
            return null;
        }

        $items = [];
        foreach ($cloze as $line) {
            if (! is_string($line) || trim($line) === '') {
                continue;
            }
            $parts = [];
            $answers = [];
            // Split keeping the {{gap}} contents: text, gap, text, gap, text…
            $tokens = preg_split('/\{\{(.+?)\}\}/', $line, -1, PREG_SPLIT_DELIM_CAPTURE);
            foreach ($tokens as $k => $tok) {
                if ($k % 2 === 0) {
                    $parts[] = (string) $this->inlineHtml($tok);
                } else {
                    $answers[] = trim($tok);
                }
            }
            if (! $answers) {
                continue; // no gap on this line
            }
            $items[] = ['parts' => $parts, 'answers' => $answers];
        }
        ...
        foreach ($questions as $q) {
            ...
            $lesson->quizQuestions()->create([
                ...
                'explanation_html' => isset($q['explanation'])
                    ...
                    : null,
                // Optional facets for the mastery dashboard.
                'tags' => $this->normalizeTags($q['tags'] ?? null) ?: null,
                'level' => isset($q['level']) ? (string) $q['level'] : null,
            ]);
        }
    }
    ...
    private function readFormationMeta(string $root, ?string $fallbackName): array
    {
        ...
        return [
            ...
            // 'formation' (default) or 'mission' — anything else falls back to 'formation'.
            'kind' => in_array(strtolower(trim((string) ($meta['kind'] ?? ''))), ['mission'], true) ? 'mission' : 'formation',
