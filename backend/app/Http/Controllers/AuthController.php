...
class AuthController extends Controller
{
    ...
    private function claimAnonymousProgress(Request $request, User $user): void
    {
        ...
        // Card reviews are unique per (owner, lesson, card): drop anonymous rows
        // that collide with the account's, attach the rest.
        $owned = CardReview::where('user_id', $user->id)
            ->get(['lesson_id', 'card_index'])
            ->map(fn ($r) => $r->lesson_id.':'.$r->card_index)
            ->flip();
        CardReview::where('client_token', $token)->whereNull('user_id')->get()
            ->each(function (CardReview $r) use ($user, $owned) {
                if (isset($owned[$r->lesson_id.':'.$r->card_index])) {
                    $r->delete();
                } else {
                    $r->forceFill(['user_id' => $user->id])->save();
                }
            });
    }
